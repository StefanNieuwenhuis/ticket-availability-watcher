...
class TestGetNewListings:
    ...
    def test_happy_path_returns_new_and_removed(self):
        ...
        current_listings = [
            self._listing(
                "new-1", listing_type="Half Marathon", seller="Bob", price="45.00"
            )
        ]

        new_listings, removed_listings = ticket_watcher.get_new_listings(
            seen_listings, current_listings
        )
    ...
    def test_same_id_with_changed_fields_is_not_new_or_removed(self):
        ...
        current_listings = [
            self._listing("same-1", seller="Alice Updated", price="30.00")
        ]
    ...
    def test_duplicate_ids_keep_last_occurrence_by_current_logic(self):
        ...
        current_listings = [self._listing("dup-1", seller="Bob", price="27.00")]
...
class TestSendTelegramNotification:
    ...
    def test_happy_path_sends_single_message(self, monkeypatch):
        ...
        ticket_watcher.send_telegram_notification(
            self._listing("abc123"), submitted=True
        )
        ...
        assert (
            ...
            == "https://example.test/event?koop=abc123"
        )
    ...
    def test_sends_multiple_messages_for_multiple_listings(self, monkeypatch):
        ...
        ticket_watcher.send_telegram_notification(
            self._listing("def456"), submitted=False
        )
    ...
    @responses.activate
    def test_missing_credentials_prints_warning_when_new_listings_exist(
        ...
        ticket_watcher.send_telegram_notification(
            self._listing("abc123"), submitted=False
        )
